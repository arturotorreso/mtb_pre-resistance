...
echo "Adding FT tag"
/SAN/ballouxlab/tb_lima/dev/addFT.py $TEMP_DIR/$SAMPLE.sfilt.vcf.gz $TEMP_DIR/$SAMPLE.sfilt.withFT.vcf;
...
/SAN/ballouxlab/tb_lima/dev/vcf2pseudoseq.py -v $VCF_FILE -o $TEMP_DIR/$SAMPLE.fa -H iupac --fill_with_N -d -w -r $REF -f "FORMAT/FT == 0" -f "FILTER != PASS"
